...
module.exports = {
  ...
  async execute(sock, msg, args, context) {
    const { from, sender, isGroup, groupMetadata, reply } = context;
    try {
      // Ensure command is used only in groups
      if (!isGroup) {
      ...
      if (mentioned.length >= 2) {
        ...
        if (groupMetadata?.participants) {
          ...
          if (participants.length >= 2) {
            ...
            return reply('❌ Not enough members to ship!');
          }
        } else {
      ...
      reply('❌ Something went wrong while shipping.');
    }
  }
};
